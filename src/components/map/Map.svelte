<script>
    ...
    import { handleFirebaseError, handleMapError, handleDataSyncError, setupGlobalErrorHandling } from '@/lib/errorHandler';
    ...
    const showUserError = (message, duration = 5000) => {
    ...
    // Función para guardar datos con debounce y manejo de errores
        ...
        saveTimeout = setTimeout(async () => {
            ...
            try {
                ...
                if (success) {
                    ...
                    const friendlyMessage = handleDataSyncError(new Error('Failed to save data'), { 
                        userId: user.uid, 
                        operation: 'save' 
                    });
    ...
    const initializeMapData = async () => {
        ...
        try {
            ...
            if (!firestoreConnected) {
                ...
                showUserError('Conectividad limitada. Los datos se guardarán localmente.', 3000);
            }
            ...
            const friendlyMessage = handleMapError(error, { operation: 'initialization' });
            showUserError(friendlyMessage);
            provinceLevels.set(new Array(PROVINCES.length).fill(0));
        } finally {
            isLoadingData = false;
    ...
    const handleLevelClick = (event) => {
        try {
            ...
            const friendlyMessage = handleMapError(error, { 
    ...
    const handleDownloadImage = async () => {
        ...
        try {
            ...
            showUserError('Error al descargar la imagen. Por favor, inténtalo de nuevo.');
        } finally {
            isDownloading = false;
        }
    };
    ...
    const handleDownloadData = () => {
        try {
            ...
            showUserError('Error al descargar los datos. Por favor, inténtalo de nuevo.');
        }
    };
    ...
    const handleResetMap = async () => {
        try {
            ...
            if ($currentUser && hasLoadedInitialData) {
                try {
                    ...
                    if (success) {
                        ...
                        const friendlyMessage = handleDataSyncError(new Error('Failed to reset map'), { 
                            userId: $currentUser.uid, 
                            operation: 'reset' 
                        });
                        showUserError(friendlyMessage);
            ...
            const friendlyMessage = handleMapError(error, { operation: 'reset' });
            showUserError(friendlyMessage);
        }
    };
